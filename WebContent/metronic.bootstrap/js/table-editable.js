var TableEditable = function() {
	...
	return {
		...
		init : function() {
			...
			jQuery('#order_table_wrapper .dataTables_filter input').addClass(
					"m-wrap medium"); // modify table search input
			jQuery('#order_table_wrapper .dataTables_length select').addClass(
					"m-wrap small"); // modify table per page dropdown
			jQuery('#order_table_wrapper .dataTables_length select').select2({
			...
			$('#order_table a.delete').live('click', function(e) {
				e.preventDefault();

				if (confirm("确认删除此记录 ？") == false) {
					return;
				}
				...
				$.ajax({
					type : "GET",
					url : "/orders/delete.do?orderId=" + $(this).attr("title"),
			...
			$('#order_table a.cancel').live('click', function(e) {
				e.preventDefault();
			...
			$('#order_table a.edit').live('click', function(e) {
				e.preventDefault();
